...
public class OutputViewController implements Initializable {
    ...
    public void onSave(){
        ...
        for (GoodsTV goodsTV:goodsTVS){
            ...
            output.setOutportId(inputId.getText());
            output.setNumber(goodsTV.getNumber());
            output.setProviderId(providerCB.getValue().getProviderId());
            output.setType(type.getText());
            output.setOperatorId(operatorCB.getValue().getOperatorId());
            ZoneId zoneId = ZoneId.systemDefault();

            ZonedDateTime zonedDateTime = datePicker.getValue().atStartOfDay(zoneId);
            Date date = Date.from(zonedDateTime.toInstant());
            output.setTime(date);
            output.setSponsor(sponsor.getText());
            output.setGoodsId(goodsTV.getGoodsId());
            output.setPrice(Long.parseLong(goodsTV.getPrice()));
            output.setComment(commit.getText());
            outputs.add(output);

        }
        ServerResponse response = outputService.insertOrUpdateOutput(outputs);
        assembleChoiceBox();
